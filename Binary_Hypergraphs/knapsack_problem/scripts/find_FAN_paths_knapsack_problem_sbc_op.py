...
if len(sys.argv) < 13:
  print("Usage: python find_FAN_paths_knapsack_problem_sbc_op.py <num_elements> <max_tot_weight_per> <min_weight> <max_weight> <min_multiplier> <max_multiplier> <network_output_filename.csv> <fitness_output_filename.csv> <boa_peaks_filename.csv> <peaks_filename.txt> <read or write: 'r' or 'w'> <wts_vals_filename.txt>")
  sys.exit(0)
...
W = 0
min_wt = int(sys.argv[3])
max_wt = int(sys.argv[4])
min_mult = int(sys.argv[5])
max_mult = int(sys.argv[6])
...
FAN_file_name = sys.argv[7]
fit_file_name = sys.argv[8]
boa_peaks_file_name = sys.argv[9]
peaks_file_name = sys.argv[10]
read_or_write = sys.argv[11]
wts_vals_file_name = sys.argv[12]
